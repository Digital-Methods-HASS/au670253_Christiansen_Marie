...
# Creating Western Europe plot
# Providing title and axis names
# Moving legend (final task)
Western_Europe_plot <- ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k, color = Entity)) +
  geom_smooth() +
...
```{r vizualizing plot-homicides, eval = TRUE}
# Separating countries and changing facet layout for better visualization
Western_Europe_plot + facet_wrap(~ Entity, ncol = 2, nrow = 3)
```
...
```{r plot-king, eval = FALSE}
# Using the existing king data set to create plot
Danish_rulers_plot <- ggplot(data = kings, mapping = aes(x = Midyear, y = Duration)) +
  geom_smooth() +
  geom_point() +
  ggtitle("Pattern of reign duration among Danish monarchs over time") +
  xlab("Year") +
  ylab("Duration of reign (years)")

print(Danish_rulers_plot)

# My ggplot is not shown in .html file. I get the error message: object 'kings' not found. I have not been able to figure out how to load the kings data set properly. The ggplot is shown in .Rmd file however. I have to change the eval to FALSE to be able to knit the rest of the document. 
```
**Answer:** While the pattern trends in the homicide rates visualizations above are descending over time, the pattern trend of the Danish monarch's reign duration is more stable and increases a bit in this visualization.  
...
**Answer:** Check out the changes in the plot above. 
...
**Answer:** In specific relation to the above visualized descending homicide rate patterns in European history, European countries seem more civilized today than back in the days.    